...
## IMPORTANT: Check Available Providers First

Before using any provider, check which API keys are set. **Only use providers
whose keys are available.** Ignore all others — don't mention them, don't suggest them.

```bash
# Run this first to see which providers are available:
echo "BRAVE:${BRAVE_API_KEY:+available}" "EXA:${EXA_API_KEY:+available}" "TAVILY:${TAVILY_API_KEY:+available}" "SERPER:${SERPER_API_KEY:+available}"
```

Wikipedia and Firecrawl (CLI) need no API key. All others require their respective key.
If a provider's key is not set, **skip it entirely** — do not attempt to call it.

## Quick Start

```bash
# Simple search (pick whichever keyed provider is available)
bun scripts/tavily.ts "your query" 5

# Semantic search (requires EXA_API_KEY)
bun scripts/exa.ts "your query" 5

# Fan-out: run available providers in parallel
bun scripts/tavily.ts "query" 5 > /tmp/sr-tavily.json &
...
### Brave Search (`scripts/brave.ts`) — requires `BRAVE_API_KEY`
...
### Exa Search (`scripts/exa.ts`) — requires `EXA_API_KEY`
...
### Tavily Search (`scripts/tavily.ts`) — requires `TAVILY_API_KEY`
...
### Serper (`scripts/serper.ts`) — requires `SERPER_API_KEY`
...
### Wikipedia (`scripts/wikipedia.ts`) — no key needed
...
### Firecrawl Scrape (`scripts/firecrawl-scrape.ts`) — uses `firecrawl` CLI or `FIRECRAWL_API_KEY`
